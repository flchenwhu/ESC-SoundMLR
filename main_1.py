...
def calculate_model_complexity(model, input_size=(3, 128, 344)):
    """计算模型的FLOPs和参数量"""
    model.eval()
    input_tensor = torch.randn(1, *input_size).cuda()
    flops, params = profile(model, inputs=(input_tensor,))
    return flops / 1e6, params / 1e6  # 转换为M单位


def measure_inference_time(model, input_size=(3, 128, 344)):
    """测量模型推理时间"""
    model.eval()
    input_tensor = torch.randn(1, *input_size).cuda()
    ...
    return np.mean(times) * 1000  # 转换为ms
...
def train_hybrid():
    ...
    model_test = my_model(model_type).cuda()
    flops, params = calculate_model_complexity(model_test)
    infer_time = measure_inference_time(model_test)
    ...
    loss_fn = HybridLoss(alpha=config.alpha, temperature=config.temperature).to(device)
    ...
    print('Freq mask number {} and length {}, and time mask number {} and length is{}'.format(
        config.freq_masks, config.freq_masks_width, config.time_masks, config.time_masks_width))
    print('*****')
    ...
    for epoch in range(num_epochs):
        ...
        for i, (_, x, label) in enumerate(train_loader):
            ...
            if i % 10 == 0:
                print(f"Epoch: {epoch + 1}/{num_epochs} Batch {i}/{len(train_loader)} Loss: {np.mean(train_loss):.4f}")
        ...
        print(
            f'Test {epoch + 1} val_loss is {np.mean(val_loss):.4f} train_acc is {train_acc:.4f} and val_acc is {val_acc:.4f}')
    ...
    print(f'Average Training Accuracy: {mean_train_acc:.4f} ± {std_train_acc:.4f}')
...
if __name__ == "__main__":
    ...
    main_path = f'results1/hybridLoss_MLA_{model_type}_ESC51'
